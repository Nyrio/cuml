 ...
 * the input suffices to be generated via `make_arima`.
 */
template <typename D>
class ArimaFixture : public Fixture {
 public:
  ArimaFixture(const TimeSeriesParams p, const ARIMAOrder o,
               const TimeSeriesScale s)
    : Fixture(), params(p), order(o), sparams(s) {}
  ArimaFixture() = delete;

 protected:
  void allocateData(const ::benchmark::State& state) override {
    data.allocate(*handle, params);
    data.arima(*handle, params, sparams, order);
  }
  ...
  ARIMAOrder order;
  TimeSeriesScale sparams;
  TimeSeriesDataset<D> data;
};  // end class ArimaFixture
